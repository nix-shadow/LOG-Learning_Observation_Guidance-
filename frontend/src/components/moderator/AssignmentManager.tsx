...
export default function AssignmentManager({ token, classId, className }: AssignmentManagerProps) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchWithCache(`/moderator/classes/${classId}/assignments`, {
        method: 'POST',
        ...authHeaders(),
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white mb-6 tracking-tight flex items-center">
        ...
        {className && <span className="text-white/40 text-base ml-3">— {className}</span>}
      </h2>
      ...
      <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-8">
        ...
        <input value={newAssignment.activity_id} onChange={e => setNewAssignment({...newAssignment, activity_id: e.target.value})}
          ...
          <tbody>
            {assignments.length === 0 ? (
              ...
              <tr key={a.id} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                ...
                <td className="py-5 px-6 text-white/60">{a.due_date ? new Date(a.due_date).toLocaleDateString() : 'No deadline'}</td>
      ...
      {viewing && (
        <div className="card-glow p-6 border border-white/10">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => { setViewing(null); setSubmissions([]); }} className="text-white/50 hover:text-white text-sm font-bold">Close ✕</button>
          </div>
              ...
              {submissions.map(s => (
                <div key={s.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-white/40">{new Date(s.submitted_at).toLocaleString()}</p>
                  </div>
