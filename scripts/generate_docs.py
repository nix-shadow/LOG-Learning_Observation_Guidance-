...
+-------------------------------------------------------------------------+""")
    ...
    add_body_p(doc, "The Go backend runs on port 6101 by default. Below is the complete endpoint reference:")
...
./server
# Backend starts on http://localhost:6101 with auto-seeded database log.db""")

    add_heading_2(doc, "11.2 Frontend Setup")
    add_code_block(doc, """cd frontend
# Create .env file: NEXT_PUBLIC_API_URL=http://localhost:6101/api
npm install
npm run dev
# Frontend starts on http://localhost:6100""")
